...
FridgeBoard splits the two files by what they actually are:
...
```bash
cd /path/to/repo
npm install --save-dev fridgeboard      # or: npm install -g fridgeboard
...
git commit -m "Import shared Markdown history into FridgeBoard notes"
```
...
git add AGENTS.md CLAUDE.md .github/copilot-instructions.md \
        .codex/instructions.md .cursor/rules/fridgeboard.mdc \
        docs/AGENT-COORDINATION.md
git commit -m "Add FridgeBoard coordination rules for agents"
```
...
go, or agents will keep doing it. The FridgeBoard block already says to use
`fridge pin` instead:
...
```bash
git add -A
git commit -m "Freeze legacy coordination files; FridgeBoard is the board"
```
...
Note that `.fridge/notes/` also contains notes that FridgeBoard itself wrote
