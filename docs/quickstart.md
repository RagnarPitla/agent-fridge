...
The transcripts were captured on an earlier build, so the `GENERATED by
agent-fridge 0.1.0` stamp inside the generated views is the version that wrote
them at the time. The layout of those views is unchanged on 0.2.2; only the
version, the identifiers, the timestamps and the state hash differ.
...
- `fridge` on your `PATH`. Nothing else: it is a single self-contained binary
  with no runtime and no interpreter.

Install it whichever way suits you. Every one of these works today:

```bash
# macOS and Linux, no toolchain needed
...
# prefer the JavaScript implementation? installs from GitHub, not from npm
npm install -g github:RagnarPitla/agent-fridge
```

Agent Fridge is **not published to the npm registry**, so `npx agent-fridge`
and `npm install -g agent-fridge` will not work. Use the `github:` form above,
or one of the other three. See [../README.md](../README.md#install) for the
full list of release assets and checksums.
...
The last two fields name the implementation and platform you installed, so they
will differ from this transcript. The Node build says `node v22.22.3` there
instead.
...
Agent Fridge first looks for explicit identity in exactly this order:

1. `--agent <name>` on the command line
2. the `FRIDGE_ACTOR` environment variable

Actor-owned mutations such as claim, release, heartbeat, handoff, and durable
notes require one of those two and otherwise exit `7` (`E_NO_SESSION`).
Read-only commands that need identity may infer the sole actor, but that
read-only inference never creates a session or renews a lease. A misspelled
explicit name is also exit `7`; only `fridge join` creates an actor.
