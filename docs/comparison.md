...
Agent Fridge Board solves one narrow problem: **several agents and humans editing the
...
- **Expiry.** A `flock` held by a process that is killed with `SIGKILL` is
  ...
  that a script forgot to delete is held forever. Agent Fridge Board claims carry a
...
- **Portability.** `flock` is not on macOS by default and is not a native
  concept on Windows. Agent Fridge Board's mutex is built on `mkdir`, which is atomic
  everywhere.
- **History.** No record survives the lock being released.

The relationship is not competitive. Agent Fridge Board uses the same idea internally:
...
development*, and they are much better than Agent Fridge Board at:
...
**What it does not solve:** branches do not solve the same problem. Agent Fridge Board
...
Branches and Agent Fridge Board also compose: the branch name is recorded as a label on
...
before Agent Fridge Board.**

If your agents can work in separate checkouts, worktrees give you something
Agent Fridge Board cannot: **actual isolation**. Not advisory coordination, not a
...
Agent Fridge Board is for the case where they do not fit, or where they fit badly
enough that everybody ends up back in one checkout anyway. The two are also
compatible: run Agent Fridge Board inside a worktree that several agents share.
...
This is why Agent Fridge Board is deliberately advisory. See
...
do it. A tracker is better than Agent Fridge Board at everything to do with planning,
...
Agent Fridge Board deliberately does no assignment or scheduling; that is an explicit
non-goal
...
Agent Fridge Board has no MCP server and does not require one. Because the protocol is
...
- **State outlives it badly.** A daemon that dies holding in-memory state loses
  it. Agent Fridge Board's state is files; if the CLI is killed mid-command the
...
- **It cannot be inspected with `cat`.** Debugging a daemon needs a client.
  Debugging Agent Fridge Board needs `ls .fridge/claims/`.
...
| Agent Fridge Board | advisory | lease + TTL | full | yes | no | one command per task |

---

## You probably do not need Agent Fridge Board if...
...
Agent Fridge Board invents none of that. Nothing on this page should be read as a claim
